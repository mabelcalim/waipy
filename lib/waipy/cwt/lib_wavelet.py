...
#---------------------------------------------------------#
        ...
        k = np.array(k)
        expnt = -(scale*k)*(k>0)
        norm = math.sqrt(scale*k[1])*(2**m/math.sqrt(m*(math.factorial(2*m-1))))*math.sqrt(n)
        pws = (pow(scale*k,m))
        pws = np.array(pws)
        daughter =[]
        for ex in expnt:
...
def wavelet(Y, dt, param, dj, s0, j1, mother):
    ...
    elif (((n1) / 2.0).is_integer()) == False:
       mat = np.concatenate((range(1,n1/2+1),range(1,n1/2)[::-1]), axis=1)	# create mirrored array
       mat = np.insert(mat,0,0)						# insert zero at the begining of the array
       mat = np.append(mat,0)						# insert zero at the end of the array
